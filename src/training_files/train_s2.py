...
class Trainer():
    ...
    def define_model(self, checkpoint_path = None):
        ...
        gfpgan_cpk = torch.load(cfg.experiment.network_d_path, map_location=torch.device('cpu'))
        discriminator.load_state_dict(gfpgan_cpk['params'])
        # change discriminator to take low/high res concatenation as input
        discriminator.conv_body[0][0] = EqualConv2d(in_channel=6, out_channel=32, kernel_size=1, stride=1, padding=0, bias=False)
        #if os.path.exists(checkpoint_path):
        #   discriminator.load_state_dict(checkpoint['discriminator_dict'])
        #   print("Loaded discriminator from {}.".format(checkpoint_path))
    ...
    def define_optimizer(self):
        ...
        optimizerG = torch.optim.Adam([{"params": params, "lr": 0.01},
                                      ...
                                      betas = (0, 0.99))
        """
        optimizerG = torch.optim.Adam(self.model.module.neural_layers.parameters(),
                                      lr = self.cfg.optimizer.glr,
                                      betas = (0, 0.99))
    ...
    def generator_match_step(self, data, outputs, iter_num):
        ...
        if iter_num >= self.cfg.experiment.add_gan:
            fake_input = torch.cat((pred[:bs], pred[bs:]), dim = 1)
            fake_g_pred = self.discriminator(fake_input)
    ...
    def train_step(self, iter_num):
        ...
        if iter_num >= self.cfg.experiment.add_gan:
            ...
            fake_input = torch.cat((outputs['rec'][:bs], outputs['rec'][bs:]), dim = 1)
            fake_d_pred = self.discriminator(fake_input.detach())
            real_input = torch.cat((outputs['gt'][:bs], outputs['gt'][bs:]), dim = 1)
            ...
            if iter_num % self.net_d_reg_every == 0:
                gt = torch.cat((outputs['gt'][:bs], outputs['gt'][bs:]), dim = 1)
        ...
        if iter_num > self.cfg.experiment.add_expr_plane and iter_num % 2 == 0:
        ...
        elif iter_num > self.cfg.experiment.add_expr_plane and iter_num % 2 == 1:
