...
class Model(torch.nn.Module):
    ...
    def get_canonical_plane(self, img_src, img_tar, mask):
        """
        ...
        """
        # This has to be img_tar since we are rendering at target pose !!!!
        neutral_sudo, neutral_mask = self.exp_loss_fn.get_neutral_render(img_tar)
        ...
        return canonical_plane, neutral_sudo, neutral_mask
    ...
    def get_apperance_plane(self, img, cam2world, intrinsics, base_plane, mask):
        ...
        id_planes = self.construct_planes(pcd, id_feat)
        id_planes = self.id_upsampler(id_planes)
        return id_planes
    ...
    def forward(self, data, mode = 'rec', iteration = 0, lpips_fn = None, exp_transfer = True):
        outputs = {}
        ...
        # canonical branch
        canonical_plane, neutral_sudo, neutral_mask = self.get_canonical_plane(img_src, img_tar, src_mask)
        ...
        if mode == 'cano':
            ...
            neutral_sudo = self.interpolate(neutral_sudo, size = rgb_low_res.shape[-2:])
            neutral_mask = self.interpolate(neutral_mask, size = rgb_low_res.shape[-2:])

            tar_mask_low_res = F.interpolate(tar_mask.float(), mode = 'nearest', size = rgb_low_res.shape[-2:])
            masked_pred = ((rgb_low_res + 1) / 2.0 * tar_mask_low_res) * 2 - 1
            masked_gt = ((self.interpolate(img_tar, rgb_low_res.shape[-2:]) + 1) / 2.0 * tar_mask_low_res) * 2 - 1
            outputs['pred'] = torch.cat((rgb_low_res * neutral_mask, masked_pred))
            outputs['gt'] = torch.cat((neutral_sudo * neutral_mask, masked_gt))
            outputs['neutral_sudo'] = neutral_sudo
            outputs['neutral_render'] = rgb_low_res[:batch_size]
            outputs['cano_input'] = self.interpolate(img_src, rgb_low_res.shape[-2:])
            outputs['cano_render'] = rgb_low_res
            outputs['cano_target'] = self.interpolate(img_tar, rgb_low_res.shape[-2:])

            outputs['target_plane'] = plane
            outputs['high_res'] = rgb_low_res
            return outputs
        ...
        if mode == 'full_rec_src':
            ...
            if iteration >= self.cfg.experiment.add_id_plane:
                id_planes = self.get_apperance_plane(img_src, src_cam2world, src_intrinsics, plane, src_mask)
                plane = plane + id_planes
                outputs['id_planes'] = id_planes

            rgb_low_res, depth_image = self.render_plane(plane, src_ray_origins, src_ray_directions)
            gt_low_res = self.interpolate(img_src, rgb_low_res.shape[-2:])

            if iteration >= self.cfg.experiment.add_gan:
                outputs['rec'] = self.interpolate(rgb_low_res, img_src.shape[-2:])
                outputs['gt'] = self.interpolate(gt_low_res, img_src.shape[-2:])
            else:
                outputs['rec'] = rgb_low_res
                outputs['gt'] = gt_low_res
        ...
        if mode == 'full_rec_tar' or mode == 'rec':
            ...
            if iteration >= self.cfg.experiment.add_gan:
                outputs['rec'] = self.interpolate(rgb_low_res, img_tar.shape[-2:])
                outputs['gt'] = self.interpolate(gt_low_res, img_tar.shape[-2:])
            else:
                outputs['rec'] = rgb_low_res
                outputs['gt'] = gt_low_res

            outputs['gt'] = self.interpolate(img_tar, (self.render_resolution, self.render_resolution))
